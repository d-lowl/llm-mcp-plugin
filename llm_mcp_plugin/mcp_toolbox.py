...
try:
    from bespoken.ui import confirm_tool_action, tool_debug, tool_error, tool_status  # type: ignore[import-not-found]
...
class MCPToolbox(llm.Toolbox):
    ...
    async def _ensure_initialized(self) -> None:
        ...
        try:
            ...
            logger.info(
                f"Initialized MCP toolbox '{self.config.name}' with {len(tools)} tools, {len(resources)} resources, {len(prompts)} prompts"
            )
    ...
    def __getattribute__(self, name: str) -> Any:
        ...
        try:
            ...
            def placeholder_method(*args: Any, **kwargs: Any) -> str:
    ...
    def method_tools(self) -> Iterator[llm.Tool]:
